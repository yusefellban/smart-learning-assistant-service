package com.MAHD.smart_learning_assistant_service.controller;

import com.MAHD.smart_learning_assistant_service.model.*;
import com.MAHD.smart_learning_assistant_service.service.ChatService;
...
public class ChatController {
    ...
    @PostMapping("/ask")
    @Operation(summary = "Ask AI", description = "Sends a user query to the chatbot and returns a response.")
    public String ask(@RequestParam String userId, @RequestParam String prompt) {
        return chatService.ask(userId, prompt);
    }

    @PostMapping("/ask/stream")
    @Operation(summary = "Ask AI (Streaming)", description = "Sends a user query and returns a streamed AI response.")
    ...
    @Operation(summary = "Get Chat History", description = "Retrieves the chat history for a specific user.")
    ...
    @Operation(summary = "Get All Chat Interactions", description = "Retrieves all chatbot interactions from users.")
    public List<ChatHistory> getAllAsks() {
        return chatService.getAllAsks();
    }


    @PutMapping("/user/feedback")
    @Operation(summary = "Submit Feedback", description = "Submits user feedback for a chatbot response.")
    ...
    public ResponseEntity<?> getFeedback(@PathVariable String userId) {
        Optional<UserFeedback> feedback = chatService.getFeedback(userId);
        return feedback.map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound().build());
    }

    @GetMapping("/users/feedback/all")
    @Operation(summary = "Get All Feedback", description = "Fetches all user feedback submitted to the system.")
